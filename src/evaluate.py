...
from config import FEATURES_DIR, MODELS_DIR, RESULTS_DIR, BATCH_SIZE, DEVICE
...
def main():
    ...
    # Identify the dynamically loaded class folders (sorted order matches train.py)
    class_folders = sorted([d for d in os.listdir(FEATURES_DIR) if os.path.isdir(os.path.join(FEATURES_DIR, d))])
    num_classes = len(class_folders)
    print("Classes mapped to labels:", {name: idx for idx, name in enumerate(class_folders)})
    ...
    print("="*50)
    report = classification_report(all_trues, all_preds, target_names=class_folders)
    print(report)
    print("="*50)
    ...
    plt.title("CNN Model: Test Set Confusion Matrix", fontsize=14, pad=15)
